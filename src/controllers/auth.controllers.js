...
export const login = async (req, res) => {
  try {
    const { email, password } = req.body; // Cambié 'correo' a 'email' y 'contraseña' a 'password'
    if (!email || !password)
      return res.status(400).send('Faltan datos obligatorios');
    ...
    const validPassword =
      user && (await bcrypt.compare(password, user.password)); // Cambié 'contraseña' a 'password'
    if (!validPassword)
